...
- Navigation evidence uses `runtime/curriculum_shared/confirmed_story_warps_v2` and `exit_routes_v2`. Retain the legacy directories for diagnosis, but never import their stale-coordinate records into v2; rebuild evidence with updated trainers. Story milestones remain once-only even when navigation recovers from backtracking.
- Every new training component, evaluator, policy-quality metric, or curriculum goal must extend the sanitized dashboard API and rendered dashboard with its current state, labels, and outcome data. Verify the live API and rendered dashboard after deployment; backend-only telemetry is incomplete.
