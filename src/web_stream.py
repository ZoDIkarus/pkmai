...
function renderMap(){const workers=state.cluster.workers||[],groups={};(state.cluster.known_tiles||[]).forEach(t=>{if(!Array.isArray(t)||t.length!==4)return;const [b,m,x,y]=t,k=`B${b} · M${m}`;(groups[k]??=[]).push({position:{valid:true,map_bank:b,map_id:m,x,y},known:true})});workers.filter(w=>w.position?.valid).forEach(w=>{const p=w.position,k=`B${p.map_bank} · M${p.map_id}`;(groups[k]??=[]).push(w)});const atlas=$('map-atlas'),legend=$('map-legend');atlas.replaceChildren();legend.replaceChildren();Object.entries(groups).forEach(([key,rows])=>{const card=make('section',undefined,'map-card'),title=make('h3',key),map=make('div',undefined,'coordinate-map');const xs=rows.map(w=>w.position.x),ys=rows.map(w=>w.position.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),rangeX=Math.max(1,maxX-minX),rangeY=Math.max(1,maxY-minY);const known=new Map(rows.filter(w=>w.known).map(w=>[`${w.position.x},${w.position.y}`,w]));known.forEach((w,key)=>{const [x,y]=key.split(',').map(Number),tile=make('div');tile.className='known-tile';tile.style.left=(10+(x-minX)/rangeX*80-3)+'%';tile.style.top=(10+(y-minY)/rangeY*80-3)+'%';tile.style.width=Math.max(6,80/rangeX*.72)+'%';tile.style.height=Math.max(6,80/rangeY*.72)+'%';map.append(tile)});rows.filter(w=>!w.known).forEach(w=>{const p=w.position,d=make('div',w.worker_id.replace('local-trainer-',''),'dot'+(w.in_battle?' battle':''));d.style.left=(10+(p.x-minX)/rangeX*80)+'%';d.style.top=(10+(p.y-minY)/rangeY*80)+'%';d.title=`${w.worker_id}: ${position(w)}`;map.append(d);legend.append(make('div',`${w.worker_id} · ${key} · ${p.x},${p.y}${w.in_battle?' · Battle':''}`))});card.append(title,make('div',`Bekannte Fläche: X ${minX}–${maxX} · Y ${minY}–${maxY}`,'muted'),map);atlas.append(card)});legend.append(make('div','Grün · bekannte Tiles'),make('div','Schwarz · Rand der bekannten Fläche'),make('div','Gelb · bekannte Warps'),make('div','Blau · bekannte Ziele'));if(!Object.keys(groups).length){atlas.append(make('div','Warte auf bekannte Overworld-Tiles.','empty'));legend.append(make('div','Noch keine persistenten Kartendaten vorhanden.','muted'))}}
