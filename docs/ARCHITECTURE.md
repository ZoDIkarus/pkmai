...
- `src/dynamic_brain.py` learns from uploaded rollout batches and atomically publishes the latest policy. It currently updates `dynamic_policy_best.pt` when the batch `mean_reward` is at least as high as the previous tracked value; independent quality promotion is not implemented yet (see `TODO.md`).
...
- `src/web_stream.py` exposes the watcher-first dashboard, `/api/watchers`, `/api/cluster-status`, `/watcher.jpg`, and `/watcher-observer`. `/watcher` is a redirect to `/`.
