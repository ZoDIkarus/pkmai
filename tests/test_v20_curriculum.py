...
class Test9_LongFullProbeHorizon(unittest.TestCase):
    ...
    def test_watcher_uses_the_normal_long_horizon(self):
        # 2026-09-07 (user): watcher back on the standard budget - the "watcher"
        # objective falls through to LONG_FULL_PROBE_STEPS (~32k), same as a
        # full run, so it resets on a step count like everything else.
