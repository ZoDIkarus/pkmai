...
function renderMap(){const workers=state.cluster.workers||[],groups={};workers.filter(w=>w.position?.valid).forEach(w=>{const p=w.position,k=`B${p.map_bank} · M${p.map_id}`;(groups[k]??=[]).push(w)});const atlas=$('map-atlas'),legend=$('map-legend');atlas.replaceChildren();legend.replaceChildren();Object.entries(groups).forEach(([key,rows])=>{const card=make('section',undefined,'map-card'),title=make('h3',key),map=make('div',undefined,'coordinate-map');const xs=rows.map(w=>w.position.x),ys=rows.map(w=>w.position.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),rangeX=Math.max(1,maxX-minX),rangeY=Math.max(1,maxY-minY);rows.forEach(w=>{const p=w.position,d=make('div',w.worker_id.replace('local-trainer-',''),'dot'+(w.in_battle?' battle':''));d.style.left=(12+(p.x-minX)/rangeX*76)+'%';d.style.top=(12+(p.y-minY)/rangeY*76)+'%';d.title=`${w.worker_id}: ${position(w)}`;map.append(d);legend.append(make('div',`${w.worker_id} · ${key} · ${p.x},${p.y}${w.in_battle?' · Battle':''}`))});card.append(title,make('div',`Bekannte Fläche: X ${minX}–${maxX} · Y ${minY}–${maxY}`,'muted'),map);atlas.append(card)});if(!Object.keys(groups).length){atlas.append(make('div','Warte auf gültige Overworld-Koordinaten.','empty'));legend.append(make('div','Positionen erscheinen nach dem nächsten Trainer-Heartbeat.','muted'))}}
